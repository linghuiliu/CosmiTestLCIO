...
namespace histmgr {
  ...
  const std::string HistogramCollection_t::__majorIndexParameterName("major");
  const std::string HistogramCollection_t::__histogramNameParameterName("histName");
  const std::string HistogramCollection_t::__defaultHistogramName("Histogram");
  //  unsigned int HistogramCollection_t::__instances=0;


  lcio::LCCollection *HistogramCollection_t::createHistograms(const std::string &collection_name, 
    ...
    throw (std::bad_alloc, std::runtime_error)
  {
    ...
    if (name_list.empty()) {
      ...
      _histogramCol->parameters().setValues(HistogramCollection_t::__histogramNameParameterName,temp);
    }
    else {
      _histogramCol->parameters().setValues(HistogramCollection_t::__histogramNameParameterName,*(const_cast<EVENT::StringVec*>(&name_list)));
    }
    
    for (UInt_t hist_i=0; hist_i<n_hist; hist_i++) {
      FloatHistogram1D *a_hist=new FloatHistogram1D(hist_i,par);
    ...
    throw (std::bad_alloc,std::runtime_error)
  {
    ...
    _histogramCol->parameters().setValues(HistogramCollection_t::__majorIndexParameterName,*index);
    _majorIndex=index;
...
//   void HistogramCollection_t::stat_add(lcio::LCCollection *a_col) {
//     if (!a_col) return;
...
//   void HistogramCollection_t::stat_copy(lcio::LCCollection *a_col) {
//     if (!a_col) return;
//     std::map<lcio::LCCollection *,int>::iterator elm=__colCounter.find(a_col);
...
//   void HistogramCollection_t::stat_remove(lcio::LCCollection *a_col) {
//     if (!a_col) return;
//     std::map<lcio::LCCollection *,int>::iterator elm=__colCounter.find(a_col);
//     assert (elm !=__colCounter.end());
//     __colCounter[a_col]--;
...
//   void HistogramCollection_t::stat_show(const std::string &label) {
...
//   }
  ...
  //std::map<lcio::LCCollection *, int > HistogramCollection_t::__colCounter;
}
