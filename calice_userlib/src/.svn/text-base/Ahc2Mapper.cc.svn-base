...
namespace CALICE {
  ...
  void Ahc2Mapper::fillModuleConnection(const lcio::LCCollection* col) {
    if (col->getParameters().getStringVal("TypeName") != "ModuleConnection") throw WrongDataFormatException("Ahc2Mapper::fillModuleConnection: expects collection of ModuleConnection objects");
    ...
    for (int i =0; i < col->getNumberOfElements();++i) {
      ...
      else throw WrongDataFormatException("Ahc2Mapper::fillModuleConnection: expects collection of LCGenericObject resp. ModuleConnection");
    }
  ...
  */
  void Ahc2Mapper::getChipChannelForModuleDescriptionID(const unsigned int id, const unsigned int moduleType, int &chip, int &channel) const {
    chip    = id / 36; //chip
    channel = id % 36; //channel
  ...
  */
  int Ahc2Mapper::correctCellIndex(const unsigned int moduleType, const int cellIndex) const {
    return cellIndex;
  }
  ...
  */
  unsigned int Ahc2Mapper::getCombinedModuleType(const unsigned int moduleType) const {
    return moduleType/2;
  }

  void Ahc2Mapper::fillModuleDescription(const lcio::LCCollection* col) {

    if (col->getParameters().getStringVal("TypeName") != "ModuleDescription") throw WrongDataFormatException("Ahc2Mapper::fillModuleDescription: expects collection of ModuleDescription objects");
    ...
    for (int i =0; i < col->getNumberOfElements();++i) {
      ...
      if (obj) {
        ...
        */
        ...
        if (moduleDescriptions.back()->getGeometricalCellIndex(iCell)) {
          ...
          CALICE::HcalCellIndex geoID( moduleDescriptions.back()->getGeometricalCellIndex(iCell) );
          ...
          if (I > maxI) maxI = I;//[0,23]
          if (J > maxJ) maxJ = J;//[0,23]
      ...
      else throw WrongDataFormatException("Ahc2Mapper::fillModuleDescription: expects collection of LCGenericObject resp. ModuleDescription");

    }

    // std::cout << "maxChip " << maxChip << " maxChn " << maxChannel << std::endl;
    ...
    for (unsigned int j = 0; j < moduleDescriptions.size(); ++j) {
      ...
      //std::cout << "individualSize " << individualSize << std::endl;
      ...
      */
      ...
      if (moduleDescriptions[j]->getGeometricalCellIndex(iCell)) {
        ...
        CALICE::HcalCellIndex geoID( moduleDescriptions[j]->getGeometricalCellIndex(iCell) );
        unsigned int I = geoID.getPadColumn();
        unsigned int J = geoID.getPadRow();
        ...
        // if (individualSize) {
        //
        //   std::cout << "getIndividualCellWidth " << moduleDescriptions[j]->getIndividualCellWidth(iCell) << std::endl;
        //   std::cout << "getIndividualCellHeight " << moduleDescriptions[j]->getIndividualCellHeight(iCell) << std::endl;
        //
  ...
  unsigned int Ahc2Mapper::countAvailable(const std::vector<bool> &vec) const {
  ...
  void Ahc2Mapper::printStats(std::ostream &ostream) const {
  ...
  const std::string Ahc2Mapper::printIfAvailable(bool available, unsigned int value) const {
  ...
  void Ahc2Mapper::print(std::ostream &ostream) const {
