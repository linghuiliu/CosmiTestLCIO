...
class writeScECALNoisyChannel : public writeLCGenericObject 
{
...
public:
  ...
  /*                                                                             */
  /*******************************************************************************/
  writeScECALNoisyChannel(std::string description) : writeLCGenericObject(description) {}
   ...
   lcio::LCCollectionVec* readFlatFile( const char* fname )
  {
	...
	CALICE::ScECALNoisyChannel* ScEcalNosyChan = new CALICE::ScECALNoisyChannel( ID0, ID1 );
	col->addElement( ScEcalNosyChan );
  ...
  {
    cout << "Program writeScECALNoisyChannel" << endl
    ...
    cout<<"    ./writeScECALNoisyChannel --inFile <your input file> --outFile <some output file name>"
    ...
    cout<<"    ./writeScECALNoisyChannel --inFile <your input file> --dbinit ${CALICE_DB_INIT} --folder \"/test_al/some/folder\""
  ...
  void screenDump( lcio::LCCollection* col )
  {
    for ( int i = 0; i != col->getNumberOfElements(); ++i )
      {
	CALICE::ScECALNoisyChannel* val = static_cast<CALICE::ScECALNoisyChannel*>( col->getElementAt( i ) );

	cout << "ID0/Layer: " <<val->getID0() << " "
	     << "ID1/Strip: " <<val->getID1() << endl; 
      }
  }
...
{
  writeScECALNoisyChannel wsv("wsv");
