...
/** Test program that writes the voltage corrections for Hcal SiPMs 
 *  to the database.<br>
 * 
 */
...
if( argc < 5) {
    std::cout << " usage: sipmvolcorr <file> <description> <since> <till> [<dbinit>]" << std::endl ;
 ...
 if(argc > 5) {
  ...
  //Initialize database
  string colNameDB("SiPMVolCorr");
  string folder( "/cd_calice/" + colNameDB ) ;

  //The LCIO collection which will be written to the db
  LCCollectionVec* sipmvolcorr_col = new LCCollectionVec( LCIO::LCGENERICOBJECT ) ;
 ...
 //The first lines
  ...
  //parse the file  
  while(true){ 
    fin >> dummy >> SiPM >> dummy >>  voltage >> dummy;
  
    //create a new SiPM Voltage Correction object
    SiPMVolCorr* sipmvolcorrMap = new SiPMVolCorr(SiPM, voltage);

    if (!fin.good()) break;  
    sipmvolcorrMap->print(std::cout);
    //add it to the collection
     sipmvolcorr_col->addElement( sipmvolcorrMap  ) ;
...
//Now start the storage into the db
  ...
  db.storeCollection(since, till,  sipmvolcorr_col, description);
