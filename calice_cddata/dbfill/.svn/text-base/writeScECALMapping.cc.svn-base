...
class writeScECALMapping : public writeLCGenericObject 
{
...
public:
  ...
  /*                                                                             */
  /*******************************************************************************/
  writeScECALMapping(std::string description) : writeLCGenericObject(description) {}
   ...
   lcio::LCCollectionVec* readFlatFile( const char* fname )
  {
    ...
    std::string line;
    int ID0, ID1, SLOT, FE, CHIP, CHANNEL, DAC;
    float DACv0, DACover;
	...
	ln >> ID0 >> ID1 >>  SLOT >> FE >> CHIP >> CHANNEL >> DAC >> DACv0 >> DACover;


	CALICE::ScECALMapping* ScEcalMip = new CALICE::ScECALMapping( ID0, ID1, SLOT, FE, CHIP, CHANNEL, DAC, DACv0, DACover );
	col->addElement( ScEcalMip );

	cout<<" ID0/Layer: "<< ID0 <<" ID0/Strip: "<< ID1
            <<" SLOT: " << SLOT << " FE: " << FE << " CHIP: " << CHIP << " CHANNEL: " << CHANNEL
	    <<" DAC: " << DAC << "DACv0: "<< DACv0 <<" DACover: "<< DACover;
  ...
  {
    cout << "Program writeScECALMapping" << endl
    ...
    cout<<"    ./writeScECALMapping --inFile <your input file> --outFile <some output file name>"
    ...
    cout<<"    ./writeScECALMapping --inFile <your input file> --dbinit ${CALICE_DB_INIT} --folder \"/test_al/some/folder\""
  ...
  void screenDump( lcio::LCCollection* col )
  {
    for ( int i = 0; i != col->getNumberOfElements(); ++i )
      {
	CALICE::ScECALMapping* val = static_cast<CALICE::ScECALMapping*>( col->getElementAt( i ) );
