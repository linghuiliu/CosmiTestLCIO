...
class writeScECALIntercalib : public writeLCGenericObject 
{
...
public:
  ...
  /*                                                                             */
  /*******************************************************************************/
  writeScECALIntercalib(std::string description) : writeLCGenericObject(description) {}
   ...
   lcio::LCCollectionVec* readFlatFile( const char* fname )
  {
    ...
    std::string line;
    int ID0, ID1;
    float calibconst, calibconst_err;
	...
	ln >> ID0 >> ID1 >> calibconst >> calibconst_err;


	CALICE::ScECALIntercalib* ScEcalMip = new CALICE::ScECALIntercalib( ID0, ID1, calibconst, calibconst_err );
	col->addElement( ScEcalMip );

	cout<<" ID0/Layer: "<< ID0 <<" ID0/Strip: "<< ID1
	    <<" calibconst: "<< calibconst <<" calibconst_err: "<< calibconst_err <<endl;
  ...
  {
    cout << "Program writeScECALIntercalib" << endl
    ...
    cout<<"    ./writeScECALIntercalib --inFile <your input file> --outFile <some output file name>"
    ...
    cout<<"    ./writeScECALIntercalib --inFile <your input file> --dbinit ${CALICE_DB_INIT} --folder \"/test_al/some/folder\""
  ...
  void screenDump( lcio::LCCollection* col )
  {
    for ( int i = 0; i != col->getNumberOfElements(); ++i )
      {
	CALICE::ScECALIntercalib* val = static_cast<CALICE::ScECALIntercalib*>( col->getElementAt( i ) );

	cout << "ID0/Layer: " <<val->getID0() << " "
	     << "ID1/Strip: " <<val->getID1() << " "
	     << "calibconst: "        <<val->getcalibconst() << " "
	     << "calibconst_err: "    <<val->getcalibconsterr() << endl;
      }
  }
