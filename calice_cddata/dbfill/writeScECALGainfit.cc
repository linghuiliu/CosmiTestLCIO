...
class writeScECALGainfit : public writeLCGenericObject 
{
...
public:
  ...
  /*                                                                             */
  /*******************************************************************************/
  writeScECALGainfit(std::string description) : writeLCGenericObject(description) {}
   ...
   lcio::LCCollectionVec* readFlatFile( const char* fname )
  {
    ...
    float dval, dval_err, slope, slope_err, setTemp;
	...
	ln >> ID0 >> ID1 >> dval >> dval_err >> slope >> slope_err >> setTemp >> nSuccess;


	CALICE::ScECALGainfit* ScEcalGain = new CALICE::ScECALGainfit( ID0, ID1, dval, dval_err, slope, slope_err, setTemp, nSuccess );
	col->addElement( ScEcalGain );

	cout<<" ID0/Layer: "<< ID0 <<" ID0/Strip: "<< ID1
	    <<" dval: "<< dval <<" dval_err: "<< dval_err <<" slope: "<< slope 
  ...
  {
    cout << "Program writeScECALGainfit" << endl
    ...
    cout<<"    ./writeScECALGainfit --inFile <your input file> --outFile <some output file name>"
    ...
    cout<<"    ./writeScECALGainfit --inFile <your input file> --dbinit ${CALICE_DB_INIT} --folder \"/test_al/some/folder\""
  ...
  void screenDump( lcio::LCCollection* col )
  {
    for ( int i = 0; i != col->getNumberOfElements(); ++i )
      {
	CALICE::ScECALGainfit* val = static_cast<CALICE::ScECALGainfit*>( col->getElementAt( i ) );

	cout << "ID0/Layer: " <<val->getID0() << " "
	     << " ID1/Strip: " <<val->getID1() << " "
             << " dval: " << val->getdval()
             << " dval_error: " <<val->getdval_error() << " "
             << " slope: " <<val->getslope()   << " "
