...
int main(int argc, char** argv ) {
  ...
  try {
    ...
    lcio::LCTime begin(2015, 8, 01, 0, 0, 00);
    ...
    for (int ilayer = 1; ilayer <= NLayer; ilayer++){
      ...
      float x = 0;
      float y = 0;
      float z = -1;//should be increased with thickness Layer
    
      UInt_t module_type = -1;

      
     //Valid for August TB CERN SPS (NLayer = 15)
      if (ilayer==1){//EBU0
	module_type = 4;
	z = -26.5;
      }
	...
	if (ilayer==3){//EBU2
      ...
      if (ilayer>=4 && ilayer<=11){//Single HBUs
	module_type = 0;
	z = 26.5 + 26.5*(ilayer - 2);
      }
      if (ilayer>=12){//2x2 HBUs
	module_type = 2;
	if(ilayer == 12) z = 26.5 + 26.5*(ilayer - 2);
	if(ilayer == 13) z = 26.5 + 26.5*(12 - 2) + 2*26.5;
	if(ilayer == 14) z = 26.5 + 26.5*(12 - 2) + 2*26.5 + 26.5*8;
	if(ilayer == 15) z = 26.5 + 26.5*(12 - 2) + 2*26.5 + 26.5*8 + 26.5*10;
      }
